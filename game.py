...
def remove_punct(text):
    ...
    out = text.translate(punctuation)
    return out
...
def display_room(room):
    ...
    print("\n" + "You are currrently in the " + room["name"] + "." + "\n")
...
def print_menu(exits):
    ...
    for key in exits:
        ...
        print_menu_line(str(key), exits[key])
...
def menu(exits):
    ...
    MenuNotDisplayed = True
    while MenuNotDisplayed:
        print_menu(exits)
        user_input = str(input())
        user_input = normalise_input(user_input)
        if is_valid_exit(exits, user_input) == True:
            MenuNotDisplayed = False
            return user_input
